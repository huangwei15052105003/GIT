	...
	public class Dorm : System.Windows.Forms.Form
	{
		...
		private void btQuery_Click(object sender, System.EventArgs e)
		{
			...
			if (textDormID.Text.Trim()=="")
				sql="select dormID as 宿舍号,phone as 电话,DMoney as 住宿费,bedNum as 床位数,chairNum as 椅子数,"+
					"deskNum as 桌子数,DRemark as 备注 from dorm";
			else
				sql="select dormID as 宿舍号,phone as 电话,DMoney as 住宿费,bedNum as 床位数,chairNum as 椅子数,"+
					"deskNum as 桌子数,DRemark as 备注 from dorm where dormID= '"+textDormID.Text.Trim()+"'";
			OleDbDataAdapter adp = new OleDbDataAdapter(sql,oleConnection1);
			ds.Clear();
			adp.Fill(ds,"dorm");
			dataGrid1.DataSource = ds.Tables["dorm"].DefaultView;
			dataGrid1.CaptionText = "共有"+ds.Tables["dorm"].Rows.Count+"条记录";
