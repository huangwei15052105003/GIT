...
namespace LithoForm
{
    public class LibF
    {
        public static void ungzip(string path, string decomPath, bool overwrite)
        {
            ...
            using (GZipStream stream = new GZipStream(new FileStream(path, FileMode.Open, FileAccess.ReadWrite), CompressionMode.Decompress))
            {
        ...
        public static bool dataGridViewToCsv(ref DataGridView dgv)
        {
            if (dgv.Rows.Count == 0)
            {
