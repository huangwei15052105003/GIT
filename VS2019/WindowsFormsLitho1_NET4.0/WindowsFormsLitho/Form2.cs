...
namespace WindowsFormsLitho
{
    public partial class ChartForm : Form
    {
        ...
        private void ChartForm_Load(object sender, EventArgs e)
        {
            ...
            倍率.Text = vScrollBar1.Value.ToString();
            zoomFactor = vScrollBar1.Value;
            mainForm.MainTrigger += PlotVector;
            mainForm.MainTrigger1 += ShowData;
            mainForm.MainTrigger2 += ExportDataGridTable;
        }
        private void PlotChart(DataTable data) //nikon预对位作图，已不用
        {
            if (data == null)
            {
                chart1.Titles.Clear();
...
*/
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            wfrmap.DrawLine(arrowPen, z * 100, z * 200, z *(100+(float)(100.0*zoomFactor/10.0)), z * 200);//int除以int，无小数，出错
            wfrmap.DrawString("标尺：100nm", new Font("宋体", z * 5f, FontStyle.Bold), Brushes.Black, z * 105, z * 205);
            ...
            for (int i = 0; i < checkedListBox1.Items.Count; i++)
            {
                ...
                {
                    row = checkedListBox1.GetItemText(checkedListBox1.Items[i]);
                    DataRow[] d = dt.Select("wfr='" + row + "'");
                    foreach (var item in d)
                    {
                        ...
                        wfrmap.DrawLine(pen1, z * x0, z * y0, z * ((x1-x0)*zoomFactor/10+x0), z * ((y1-y0)*zoomFactor/10+y0));
        ...
        private void PlotVector()   //主窗体调用作图，固定1000000倍率，所有圆片
        {
            ...
            for (int k = 0; k < dataGridView1.Rows.Count - 1; k++) //datagridview1 有一空行，循环减1
        ...
        private void vScrollBar1_Scroll(object sender, ScrollEventArgs e) //放大倍率，范围1-100；10代表1000000倍，即原始数据的倍率
        {
            zoomFactor = vScrollBar1.Value;
            倍率.Text = vScrollBar1.Value.ToString();
