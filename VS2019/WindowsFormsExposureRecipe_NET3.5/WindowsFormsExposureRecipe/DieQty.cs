...
namespace Map
{
    class DieQty
    {
        ...
        public static DataTable CalculatOffset(double wee, double xImg, double stepX, double stepY, double dieX, double dieY, double pricision, string parttype)
        {
            ...
            if (laserY_Flag == false)
            {
                ...
                dt.Clear();
                DataRow newRow = dt.NewRow();
                newRow["shiftX"] = System.Math.Round(offX, 3);
                newRow["shiftY"] = System.Math.Round(offY, 3);
                newRow["dieQty"] = totalDie;

                //3mm Coverage
                newRow["leftPartialSize"] = System.Math.Round((99.5 - stepX / 2 + offX) % stepX, 3);
                newRow["rightPartialSize"] = System.Math.Round((99.5 - stepX / 2 - offX) % stepX, 3);
                newRow["miniSizeX"] = System.Math.Round(0.5 * stepX - 3.3999, 3);
        ...
        public static void Plot(Graphics wfrmap, float k, float sx, float sy, float dx, float dy, float ox, float oy, double areaRatio, bool fullCover, string parttype, bool nikonFlag,string savePath,string part)
        {
            #region 参数和变量

            if (parttype == "largeField")
            { float tmp = sx; sx = sy; sy = 2 * tmp; tmp = dx; dx = dy; dy = tmp; }
            ...
            //数组写入文本文件
            string txtFileName = savePath + ".txt";
        ...
        //普通视场，原点左上角，画图用
        private static bool LaserMarkCheck(double x, double y)
        {
        ...
        private static bool NotchCheck(double x, double y)
        {
        ...
        //大视场，原点左上角，画图用
        private static bool LaserMarkCheckLarge(double x, double y)
        {
        ...
        private static bool NotchCheckLarge(double x, double y)
        {
        ...
        //普通视场，原点中心，计算offset用
        private static bool LaserMarkCheck1(double x, double y)
        {
        ...
        private static bool NotchCheck1(double x, double y)
        {
        ...
        //大视场,原点中心，计算offset用
        private static bool LaserMarkCheck1_Large(double x, double y)
        {
        ...
        private static bool NotchCheck1_Large(double x, double y)
        {
        ...
        private static bool AreaCalculation(bool s1, bool s2, bool s3, bool s4, double llx, double lly, double sx, double sy,double areaRatio)
        {
